...
<?php $component = Illuminate\View\AnonymousComponent::resolve(['view' => 'components.layout-app','data' => ['title' => 'Dashboard & Analisis','role' => $role]] + (isset($attributes) && $attributes instanceof Illuminate\View\ComponentAttributeBag ? $attributes->all() : [])); ?>
...
<?php endif; ?>
<?php $component->withAttributes(['title' => 'Dashboard & Analisis','role' => \Illuminate\View\Compilers\BladeCompiler::sanitizeComponentAttribute($role)]); ?>
        ...
        <div class="grid grid-cols-2 md:grid-cols-4 gap-3">
            <div class="bg-white p-4 rounded-xl border border-slate-100 flex justify-between items-center shadow-sm">
                <div>
                    ...
                    <h3 class="text-xl font-bold text-slate-800 mt-0.5"><?php echo e($statistik['total_siswa'] ?? 0); ?></h3>
                </div>
            ...
            <div class="bg-white p-4 rounded-xl border border-slate-100 flex justify-between items-center shadow-sm">
                <div>
                    ...
                    <h3 class="text-xl font-bold text-green-500 mt-0.5"><?php echo e($statistik['hadir_hari_ini'] ?? 0); ?></h3>
                </div>
            ...
            <div class="bg-white p-4 rounded-xl border border-slate-100 flex justify-between items-center shadow-sm">
                <div>
                    ...
                    <h3 class="text-lg font-bold text-indigo-500 mt-0.5"><?php echo e($statistik['rata_rata_kedatangan'] ?? '00:00'); ?></h3>
                </div>
            ...
            <div class="bg-white p-4 rounded-xl border border-slate-100 flex justify-between items-center shadow-sm">
                <div>
                    ...
                    <h3 class="text-xl font-bold text-red-500 mt-0.5"><?php echo e($statistik['terlambat_hari_ini'] ?? 0); ?></h3>
                </div>
...
<div class="grid grid-cols-1 lg:grid-cols-3 gap-4">
    ...
    <div class="bg-white rounded-xl border border-slate-100 p-5 shadow-sm flex flex-col">
        ...
        <div class="flex-1 overflow-y-auto custom-scroll space-y-2 max-h-[200px]">
            <?php $__empty_1 = true; $__currentLoopData = $statistik['siswaTerlambatTerkini'] ?? []; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $telat): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); $__empty_1 = false; ?>
                <div class="flex items-center justify-between p-2.5 bg-red-50 rounded-lg border border-red-100">
                    <div class="flex items-center gap-2.5">
                        <div class="w-8 h-8 rounded-md bg-red-200 text-red-700 flex items-center justify-center font-bold text-[11px]">
                            <?php echo e($telat['kelas'] ?? '-'); ?>

                        </div>
                        <div>
                            <p class="font-bold text-slate-800 text-xs"><?php echo e($telat['nama'] ?? 'Siswa'); ?></p>
                            <p class="text-[9px] text-red-500 font-bold">Telat <?php echo e($telat['menit_terlambat'] ?? 0); ?> Menit</p>
                        </div>
                    </div>
                    <span class="text-[10px] font-mono font-bold text-slate-500"><?php echo e($telat['waktu_datang'] ?? '00:00'); ?></span>
                </div>
    ...
    <div class="bg-white rounded-xl border border-slate-100 p-5 shadow-sm flex flex-col">
        ...
        <div class="flex-1 space-y-2">
            ...
            <?php $__empty_1 = true; $__currentLoopData = $statistik['siswaRajin'] ?? []; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $index => $rajin): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); $__empty_1 = false; ?>
                <div class="flex items-center justify-between p-2.5 bg-emerald-50/50 rounded-lg border border-emerald-100 transition hover:scale-[1.02]">
                    <div class="flex items-center gap-3">
                        <div class="w-8 h-8 rounded-full <?php echo e($medals[$index]['bg'] ?? 'bg-blue-100'); ?> <?php echo e($medals[$index]['text'] ?? 'text-blue-600'); ?> flex items-center justify-center text-xs shadow-sm">
                            <i class="fa-solid <?php echo e($medals[$index]['icon'] ?? 'fa-award'); ?>"></i>
                        </div>
                        <div>
                            <p class="font-extrabold text-slate-800 text-xs uppercase"><?php echo e($rajin['nama'] ?? 'Siswa'); ?></p>
                            <p class="text-[9px] text-slate-500 font-bold tracking-tight">Kelas <?php echo e($rajin['kelas'] ?? '-'); ?></p>
                        </div>
                    </div>
                    <div class="text-right">
                        <p class="text-[11px] font-black text-emerald-600"><?php echo e($rajin['total_tepat_waktu'] ?? 0); ?>x</p>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function () {
            const statData = <?php echo json_encode($statistik, 15, 512) ?>;
            const trenData = <?php echo json_encode($tren, 15, 512) ?>;

            const dataHarian   = <?php echo json_encode($rekapHarian, 15, 512) ?>;
            const dataMingguan = <?php echo json_encode($rekapMingguan, 15, 512) ?>;
            const dataBulanan  = <?php echo json_encode($rekapBulanan, 15, 512) ?>;
            ...
            let tahunAjaranList = <?php echo json_encode($taList ?? [], 15, 512) ?>;
            ...
            async function fetchTahunAjaran() {
                try {
                    const res = await fetch('<?php echo e(route("admin.tahun_ajaran.list")); ?>', {
            ...
            async function tambahTahunAjaran() {
                ...
                try {
                    const res = await fetch('<?php echo e(route("admin.tahun_ajaran.store")); ?>', {
                        method: 'POST',
                        headers: {
            ...
            async function aktifkanTahunAjaran(id) {
                try {
                    const res = await fetch(`<?php echo e(url('admin/tahun_ajaran/aktifkan')); ?>/${id}`, {
                        method: 'PATCH',
                        headers: {
                            'X-CSRF-TOKEN': '<?php echo e(csrf_token()); ?>',
                            'X-Requested-With': 'XMLHttpRequest',
...
<?php endif; ?>
<?php /**PATH C:\laragon\www\New foldersssss\ekesiswaan\resources\views/admin/dashboard.blade.php ENDPATH**/ ?>