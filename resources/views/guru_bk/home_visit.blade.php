<x-layout-app title="Home Visit" :role="$role">
    ...
    <div class="w-full space-y-4 pb-6 fade-in">
        ...
        <div class="bg-white p-4 lg:p-5 rounded-2xl shadow-sm border border-slate-200 flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
            ...
            <div class="flex items-center gap-2 w-full md:w-auto">
                {{-- Form Filter Kelas (Sudah pakai auto-submit AJAX) --}}
                <form id="filterFormVisit" method="GET" action="{{ route('bk.visit') }}" class="relative group flex-1 md:flex-none">
        ...
        <div id="list-container" class="grid grid-cols-1 lg:grid-cols-2 gap-6 transition-opacity duration-300">
            ...
            <div class="flex flex-col gap-3">
                <div class="p-3 bg-[#f0fdf4] border border-[#bbf7d0] rounded-xl flex justify-between items-center shadow-sm">
                    ...
                    <span class="bg-white text-[#16a34a] border border-[#86efac] px-2.5 py-1 rounded-md text-[10px] font-bold shadow-sm">{{ count($visitSelesai) }} Selesai</span>
                </div>
                ...
                @forelse($visitSelesai as $visit)
                    ...
                    @endphp
                    <div onclick="openDetailModal({{ $visit['id'] }})" class="bg-white rounded-xl border border-slate-200 p-4 shadow-sm flex justify-between items-center cursor-pointer hover:border-slate-300 hover:shadow-md transition group">
                        <div class="flex gap-3 items-center">
                            ...
                            <div>
                                <p class="text-[12px] font-bold text-slate-800 group-hover:text-[#2563eb] transition">
                                    {{ $visit['nama_siswa'] }}
                                    <span class="ml-1 text-[9px] font-normal text-slate-400">({{ $visit['kelas'] }})</span>
                                </p>
                                <p class="text-[10px] text-slate-400 mt-0.5 flex items-center gap-1.5"><i class="fa-regular fa-calendar"></i> {{ $tglDisplay }}</p>
                            </div>
                        </div>
                        <div class="flex gap-1.5 text-sm flex-shrink-0">
                            <button onclick="event.stopPropagation(); openEditModal({{ json_encode($visit) }}, '{{ $tglEdit }}')" class="w-7 h-7 rounded-lg bg-slate-50 border border-transparent text-slate-400 hover:text-[#2563eb] hover:border-slate-200 transition flex items-center justify-center text-[11px]" title="Edit">
                                <i class="fa-regular fa-pen-to-square"></i>
                            </button>
                            <button onclick="event.stopPropagation(); confirmDelete({{ $visit['id'] }})" class="w-7 h-7 rounded-lg bg-slate-50 border border-transparent text-slate-400 hover:text-[#ef4444] hover:border-slate-200 transition flex items-center justify-center text-[11px]" title="Hapus">
            ...
            {{-- ================= LOOPING: RENCANA KUNJUNGAN (PENDING) ================= --}}
            <div class="flex flex-col gap-3">
                <div class="p-3 bg-[#fff7ed] border border-[#ffedd5] rounded-xl flex justify-between items-center shadow-sm">
                    ...
                    <span class="bg-white text-[#ea580c] border border-[#fdba74] px-2.5 py-1 rounded-md text-[10px] font-bold shadow-sm">{{ count($visitPending) }} Pending</span>
                </div>
                        ...
                        <div class="flex gap-3 items-center">
                            ...
                            <div>
                                ...
                                <p class="text-[10px] text-[#ea580c] font-medium mt-0.5 flex items-center gap-1.5">
                                    <i class="fa-regular fa-calendar"></i> {{ $tglDisplay }}
                                </p>
                            </div>
                        </div>
                        <div class="flex gap-1.5 text-sm flex-shrink-0">
    ...
    <script>
        @if(session('success'))
            Swal.fire({ title: 'Berhasil!', text: '{{ session("success") }}', icon: 'success', timer: 1500, showConfirmButton: false });
        @endif
        @if(session('error'))
