...
<aside id="sidebar" class="w-72 bg-navy-900 text-white flex flex-col z-50 shadow-2xl flex-shrink-0 fixed top-0 left-0 h-full transition-transform duration-300 ease-in-out">
    ...
    <div class="h-20 flex items-center px-8 border-b border-navy-800/50 bg-navy-900 justify-between flex-shrink-0">
        <div class="flex items-center gap-3">
            ...
            <div>
                ...
                <p class="text-[10px] text-slate-400 font-medium tracking-widest uppercase">SMP NEGERI 1 POLANHARJO</p>
            </div>
        </div>
    ...
    <div class="p-6 flex-shrink-0">
        <a href="{{ $role == 'kepala_sekolah' ? '/kepsek/profile' : ($role == 'guru_bk' ? '/bk/profile' : '/admin/profile') }}"
           class="p-4 bg-navy-800/40 rounded-2xl border border-navy-700/50 flex items-center gap-3 cursor-pointer hover:bg-navy-800 transition group block">
            <div class="relative">
                <div class="w-12 h-12 rounded-full {{ $role == 'kepala_sekolah' ? 'bg-indigo-600' : ($role == 'guru_bk' ? 'bg-blue-600' : 'bg-orange-500') }} flex items-center justify-center font-bold text-white shadow-lg border-2 border-navy-900">
            ...
            <div class="overflow-hidden">
                <p class="text-sm font-bold text-white group-hover:text-indigo-200 transition">
                    @if($role == 'kepala_sekolah') Bpk. Kepala Sekolah
                    @elseif($role == 'guru_bk') Guru BK
                    @else Admin / Operator
                    @endif
                </p>
                <p class="text-[10px] text-slate-400">Edit Profil</p>
            </div>
        </a>
    </div>
    ...
    <div class="p-6 border-t border-navy-800/50 flex-shrink-0">
        <form action="{{ route('logout') }}" method="POST">
            @csrf
            <button type="submit"
                class="w-full flex items-center justify-center gap-2 text-slate-400 hover:text-red-400 hover:bg-navy-800 py-3 rounded-xl transition-all text-xs font-bold uppercase tracking-wider group">
