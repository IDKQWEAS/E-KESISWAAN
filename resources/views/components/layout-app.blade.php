...
<body class="bg-surface text-slate-600 font-sans overflow-hidden">
    ...
    {{-- SIDEBAR COMPONENT --}}
    <x-sidebar :role="$role ?? session('user_data.role', 'admin')" />
    ...
    <div id="main-wrapper" class="relative flex flex-col h-screen transition-all duration-300 lg:pl-72 w-full">
        ...
        <header class="h-16 lg:h-20 flex-shrink-0 px-5 lg:px-8 flex items-center justify-between bg-white/90 backdrop-blur-md z-30 sticky top-0 border-b border-slate-200/80 shadow-sm">
            <div class="flex items-center gap-4 min-w-0">
                ...
                <h2 class="text-lg lg:text-xl font-bold text-slate-800 tracking-tight truncate">{{ $title ?? 'Dashboard' }}</h2>
            </div>
                ...
                <div class="flex items-center gap-3 flex-shrink-0 pl-4">
                    ...
                    @if(!empty($listTahunAjaran))
                        <form method="GET" action="{{ url()->current() }}" id="formTahunAjaran">
                            {{-- Bawa parameter lain, tapi REVISI: hapus parameter 'page' agar paginasi ke-reset saat ganti TA --}}
                            @foreach(request()->except(['id_tahun_ajaran', 'page']) as $key => $val)
                            ...
                            <select id="selectTahunAjaran" name="id_tahun_ajaran"
                                ...
                                @foreach($listTahunAjaran as $ta)
                                    ...
                                    @endphp
                                    <option value="{{ $ta['id'] }}" {{ $selected ? 'selected' : '' }}>{{ $label }}</option>
                                @endforeach
                            </select>
                        </form>
                    @else
