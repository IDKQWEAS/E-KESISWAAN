...
<body class="bg-[#f8fafc] font-sans text-slate-900 antialiased">
        ...
        <header class="h-16 bg-white border-b border-slate-200 flex items-center justify-between px-4 md:px-8 sticky top-0 z-30 shadow-sm">
            <div class="flex items-center gap-2 md:gap-3 flex-shrink min-w-0">
                <div class="w-8 h-8 md:w-10 md:h-10 rounded-full flex items-center justify-center overflow-hidden flex-shrink-0 border border-slate-100">
                    <img src="{{ asset('smp.png') }}" alt="Logo" class="w-full h-full object-cover">
                </div>
                <div class="min-w-0">
                    <h1 class="text-xs md:text-sm font-extrabold text-slate-800 uppercase truncate">Monitoring {{ $tingkat ? 'Kelas ' . $tingkat : 'Siswa' }}</h1>
            ...
            <div class="flex items-center gap-2 md:gap-3 flex-shrink-0">
                <select onchange="window.location.href='?tingkat='+this.value" class="bg-slate-50 border border-slate-200 rounded-lg py-1.5 px-2 text-[9px] font-black text-slate-700 uppercase outline-none cursor-pointer">
                    ...
                    <option value="7" {{ $tingkat == '7' ? 'selected' : '' }}>KELAS 7</option>
                    <option value="8" {{ $tingkat == '8' ? 'selected' : '' }}>KELAS 8</option>
                    <option value="9" {{ $tingkat == '9' ? 'selected' : '' }}>KELAS 9</option>
                </select>
                    ...
                    <table class="w-full text-left border-collapse min-w-[750px]">
                        <thead class="bg-slate-50/50 border-b border-slate-100 sticky top-0 z-10 backdrop-blur-md">
                            <tr class="text-[9px] font-black text-slate-400 uppercase tracking-widest">
                                ...
                                <th class="px-5 py-4 text-center w-32">Tipe</th> {{-- KOLOM BARU --}}
                            ...
                            @forelse($riwayatAbsen as $row)
                                ...
                                <tr class="hover:bg-slate-50/50 transition-colors h-14">
                                    <td class="px-5 text-center font-mono font-bold text-slate-500 text-[10px]">
                                        {{ \Carbon\Carbon::parse($row['created_at'])->timezone('Asia/Jakarta')->format('H:i') }}
                                    </td>
                                    <td class="px-5">
                                        <div class="flex flex-col">
                                            <span class="text-[11px] font-extrabold text-slate-800 uppercase leading-tight">{{ $row['nama'] }}</span>
                                            <span class="text-[8px] font-bold text-blue-500 uppercase tracking-wider">NIPD: {{ $row['nipd'] }}</span>
                                        </div>
                                    </td>
                                    <td class="px-5 text-center font-black text-slate-600 text-[10px]">
                                        {{ $row['kelas'] }}
                                    </td>
                                    {{-- Kolom Status (Jenis Absen) --}}
                                    <td class="px-5 text-center">
                                        <span class="{{ $statusColor }} text-white px-2 py-1 rounded-full text-[8px] font-black uppercase shadow-sm">
                                            {{ strtoupper($tipeRaw) }}
                                        </span>
                                    </td>
                                    {{-- Kolom Tipe (Ketepatan Waktu) --}}
                                    <td class="px-5 text-center">
                                        <span class="{{ $tipeColor }} text-white px-2 py-1 rounded-full text-[8px] font-black uppercase shadow-sm">
                                            {{ $tipeLabel }}
                                        </span>
                                    </td>
                                    <td class="px-5 text-center">
                                        <span class="px-2 py-1 bg-orange-50 text-orange-600 rounded text-[10px] font-black">{{ $row['total_terlambat'] ?? 0 }}x</span>
                                    </td>
                                    <td class="px-5 text-center font-black text-red-600 text-[10px]">{{ $row['total_poin'] ?? 0 }}</td>
                                </tr>
                            @empty
    ...
    <script>
        ...
        async function cancelAttendance() {
            ...
            if (nipd) {
                const dataRiwayat = @json($riwayatAbsen);
        ...
        async function handleTandaiAlpha() {
            ...
            if (result.isConfirmed) {
                Swal.fire({ title: 'Memproses...', didOpen: () => Swal.showLoading(), allowOutsideClick: false });
                try {
                    const res = await fetch("{{ route('absensi.tandaiAlpha') }}", {
                        method: 'POST',
                        headers: { 'Content-Type': 'application/json', 'X-CSRF-TOKEN': '{{ csrf_token() }}' }
                    });
                    const data = await res.json();
